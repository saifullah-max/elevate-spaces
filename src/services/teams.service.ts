...
export const allocateCreditsToMember = async (
    ...
    try {
        ...
        if (axios.isAxiosError(error)) {
            throw {
                code: error.response?.data?.code,
                data: error.response?.data?.data || error.response?.data?.payload,
                message:
                    error.response?.data?.message ||
                    error.message ||
...
export const getTeamEligibility = async (teamId: string): Promise<any> => {
    try {
        if (!API_BASE_URL) throw new Error('Backend API URL is not configured');
        const response = await axios.get<any>(`${API_BASE_URL}/teams/eligibility/${teamId}`, { headers: getAuthHeaders() });
        return response.data;
    } catch (error) {
        if (axios.isAxiosError(error)) {
            throw {
                message: error.response?.data?.message || error.message || 'Failed to fetch team eligibility',
                code: error.response?.data?.code,
