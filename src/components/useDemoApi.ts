    ...
    const handleStageMultipleImages = async (
        ...
        return new Promise<{ total: number; imageIds: string[]; creditsUsed?: number; creditScope?: "team" | "personal" } | null>((resolve) => {
            stageMultipleImagesSSE({
                ...
                roomType: areaType === "interior" ? roomType : exteriorType,
                stagingStyle,
                roomTypes: areaType === "interior" && perImageRoomTypes ? perImageRoomTypes.filter((rt): rt is RoomType => rt !== undefined) : undefined,
                stagingStyles: perImageStagingStyles ? perImageStagingStyles.filter((s): s is StagingStyle => s !== undefined) : undefined,
                prompt,
                removeFurniture,
                deviceId,
                teamId,
