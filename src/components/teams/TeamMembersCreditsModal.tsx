...
export function TeamMembersCreditsModal({
  ...
  const [removeModalMember, setRemoveModalMember] = useState<any>(null);
      ...
      <DialogContent className="sm:max-w-5xl max-h-[85vh] flex flex-col overflow-y-scroll">
        ...
        <div className="flex-1 px-6 py-4">
          <div className="space-y-6">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {activeMembers.map((member) => {
                        ...
                        const removalInvite = team.teamInvites?.find(
                          (invite) => invite.accepted_by_user_id === member.user_id && invite.status === "ACCEPTED"
                        );
                        const removalId = removalInvite?.id || member.id;
                        const currentUserMember = team.members.find(m => m.user_id === currentUserId);
                        const currentUserIsOwner = currentUserId === team.owner_id;
                        const currentUserIsAdmin = currentUserMember?.role?.name === "TEAM_ADMIN";
                        const canEditRole = onUpdateMemberRole && !isOwner;
                        // Owner can remove any member (except owner cannot remove themselves here),
                              ...
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent align="start">
                                  ...
                                  {canRemove && (
                                    <DropdownMenuItem
                                      onClick={() => {
                                        setRemoveModalMember({ ...member, removalId });
            ...
            {removeModalOpen && removeModalMember && (
              <Dialog open={removeModalOpen} onOpenChange={setRemoveModalOpen}>
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    <DialogTitle>Remove from team</DialogTitle>
                    <DialogDescription>
                      Remove <span className="font-semibold">{removeModalMember.user.email}</span> from {team.name}?
                    </DialogDescription>
                  </DialogHeader>
                  <div className="mt-4 rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                    This will immediately revoke their team access. Admins cannot remove the owner.
                  </div>
                  <div className="flex justify-end gap-2 mt-6">
                    ...
                    <Button
                      variant="destructive"
                      disabled={removingMemberId === removeModalMember.removalId || removingMemberId === removeModalMember.id}
                      onClick={async () => {
                        try {
                          await Promise.resolve(onRemoveMember?.(removeModalMember.removalId || removeModalMember.id, team.id, team.owner_id));
                    ...
                    >
                      {removingMemberId === removeModalMember.id ? "Removing..." : "Remove"}
                    </Button>
                  </div>
                </DialogContent>
              </Dialog>
