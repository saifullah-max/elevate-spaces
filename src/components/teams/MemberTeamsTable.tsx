...
export function MemberTeamsTable({ teams, currentUserId, onInviteClick, onTransferClick, onLeaveClick, onViewAllClick, onAllocateCreditsClick, onEditNameClick, onDeleteClick }: MemberTeamsTableProps) {
    ...
    const ALLOWED_ROLES = ["TEAM_OWNER", "ADMIN", "MEMBER", "PHOTOGRAPHER", "TEAM_ADMIN", "TEAM_MEMBER", "TEAM_PHOTOGRAPHER"];
    ...
    const canViewTeamWallet = (team: Team) => {
        const currentRoleName = getCurrentRoleName(team);
        return ["TEAM_OWNER", "ADMIN", "TEAM_ADMIN"].includes(currentRoleName || "");
    };

    const canSeeAllocatedCredits = (team: Team) => {
        const currentRoleName = getCurrentRoleName(team);
        return ["MEMBER", "PHOTOGRAPHER", "TEAM_MEMBER", "TEAM_PHOTOGRAPHER"].includes(currentRoleName || "");
    };

    const getDisplayedCredits = (team: Team) => {
        const currentRoleName = getCurrentRoleName(team);
        const member = team.members.find(mem => mem.user_id === currentUserId);
        const canUseWallet = ["TEAM_OWNER", "ADMIN", "TEAM_ADMIN"].includes(currentRoleName || "");

        return canUseWallet ? Number(team.wallet || 0) : Number(member?.allocated || 0);
    };
    ...
    const isTeamAgentInTeam = (team: Team) => {
        ...
        return currentRoleName === "MEMBER" || currentRoleName === "TEAM_MEMBER";
    };
