    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <StatCard label="Total users" value={users.length} icon={<Users className="h-5 w-5 text-indigo-600" />} />
          ...
          <table className="min-w-full divide-y divide-slate-200 text-left">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">First accepted</th>
                <th className="px-6 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Recent accepted</th>
              </tr>
            </thead>
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50/70">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDate(user.aiConsentFirstAcceptedAt)}</td>
                    <td className="px-6 py-4 text-sm text-slate-600">{formatDate(user.aiConsentRecentAcceptedAt)}</td>
                  </tr>
                ))
              )}
            </tbody>
