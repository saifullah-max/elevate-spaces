  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-600">Edit the public resources page from one place.</p>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-5 flex flex-col gap-3 border-b border-slate-200 pb-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">{selectedResource?.title || "Resources"}</h2>
            <p className="text-sm text-slate-600">Manage text, PDF and YouTube link for the public resources page.</p>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            />
            <p className="mt-1 text-xs text-slate-500">Paste a YouTube link instead of uploading a video file.</p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 rounded-t-xl border border-slate-200 bg-slate-50 p-2">
              <button type="button" onClick={() => editor?.chain().focus().toggleHeading({ level: 2 }).run()} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">H2</button>
              <button type="button" onClick={() => editor?.chain().focus().toggleBold().run()} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Bold</button>
              <button type="button" onClick={() => editor?.chain().focus().toggleItalic().run()} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Italic</button>
              <button type="button" onClick={() => editor?.chain().focus().toggleBulletList().run()} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Bullets</button>
              <button type="button" onClick={() => editor?.chain().focus().toggleOrderedList().run()} className="rounded-md bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100">Numbered</button>
            </div>
...
function AssetUploadCard({
  ...
  return (
    <div className="rounded-xl border border-slate-200 p-4">
      ...
      <label className="mt-4 flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-300 px-4 py-5 text-center hover:border-indigo-500 hover:bg-indigo-50/40">
        <Upload className="h-5 w-5 text-slate-500" />
        <span className="mt-2 text-sm font-medium text-slate-700">Choose {title.toLowerCase()}</span>
        <span className="mt-1 text-xs text-slate-500">{file ? file.name : showCurrentFile ? currentName || "No file selected" : "No file selected"}</span>
      ...
      <div className="flex flex-row align-middle gap-6 mt-4">
        {showCurrentFile && currentUrl ? (
          <Link href={currentUrl} target="_blank" className="pt-2 inline-flex text-sm font-medium text-indigo-600 hover:underline">
