...
function PhotographerSignUpForm() {
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-linear-to-br from-slate-50 to-white">
      <div className="w-full max-w-3xl p-8 space-y-8 rounded-2xl bg-white shadow-lg border border-slate-200">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">Create your photographer account</h1>
          <p className="text-slate-600 mt-2">Join the marketplace and submit your photographer profile in one step</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          {fromDemoBonus && !success && (
            <div className="p-3 bg-linear-to-r from-indigo-50 to-blue-50 border border-indigo-200 rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-semibold text-indigo-900">Bonus Offer: You'll receive 5 free credits instantly!</p>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input name="name" placeholder="Enter your full name" value={form.name} onChange={handleChange} required className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Email</label>
              <Input name="email" type="email" placeholder="you@example.com" value={form.email} onChange={handleChange} required className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
            </div>
          ...
          <div className="space-y-4 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
            <div>
              <h3 className="text-sm font-semibold text-indigo-900">Photographer onboarding details</h3>
              <p className="text-xs text-indigo-800 mt-1">These details help the admin review your application faster.</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              <Input name="businessName" value={photographerForm.businessName} onChange={handlePhotographerChange} placeholder="Business name" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="photographerType" value={photographerForm.photographerType} onChange={handlePhotographerChange} placeholder="Photographer type (real estate, interiors, drone)" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="yearsExperience" value={photographerForm.yearsExperience} onChange={handlePhotographerChange} placeholder="Years of experience" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="serviceArea" value={photographerForm.serviceArea} onChange={handlePhotographerChange} placeholder="Primary service area" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="portfolioUrl" value={photographerForm.portfolioUrl} onChange={handlePhotographerChange} placeholder="Portfolio URL" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="instagramUrl" value={photographerForm.instagramUrl} onChange={handlePhotographerChange} placeholder="Instagram URL" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="websiteUrl" value={photographerForm.websiteUrl} onChange={handlePhotographerChange} placeholder="Website URL" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
              <Input name="availability" value={photographerForm.availability} onChange={handlePhotographerChange} placeholder="Availability placeholder" className="h-11 border-slate-300 focus:border-indigo-600 focus:ring-indigo-600" />
            </div>

            <textarea name="bio" value={photographerForm.bio} onChange={handlePhotographerChange} className="min-h-28 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600" placeholder="Short professional bio" />
            <textarea name="shortPitch" value={photographerForm.shortPitch} onChange={handlePhotographerChange} className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600" placeholder="Why should clients hire you?" />
            <textarea name="gearDescription" value={photographerForm.gearDescription} onChange={handlePhotographerChange} className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600" placeholder="Camera / gear / equipment details" />
          </div>

          <Button type="submit" className="w-full h-11 font-semibold bg-indigo-600 hover:bg-indigo-700 text-white shadow-md hover:shadow-lg transition-all duration-200" disabled={loading}>
            {loading ? (
        ...
        <div className="mt-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3">
            <Button variant="outline" className="h-11 border-slate-300 hover:bg-slate-50 text-slate-700 hover:border-slate-400" type="button" onClick={handleGoogleSignUp}>
            ...
            <Button variant="outline" className="h-11 border-slate-300 hover:bg-slate-50 text-slate-700 hover:border-slate-400">
        ...
        <div className="text-center pt-4">
          <p className="text-slate-600 text-sm">
            ...
            <Link href="/sign-in" className="font-semibold text-indigo-600 hover:text-indigo-700 hover:underline transition-colors">
              Sign in
            </Link>
          </p>
          <p className="mt-2 text-xs text-slate-500">
            Want a normal user account?{" "}
            <Link href="/sign-up" className="font-semibold text-indigo-600 hover:text-indigo-700 hover:underline transition-colors">
              Sign up as user
            </Link>
          </p>
          <p className="text-xs text-slate-500 mt-3">By signing up, you agree to our Terms and Privacy Policy</p>
        </div>
      </div>
    </div>
  );
