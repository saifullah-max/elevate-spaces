...
function SignUpForm() {
  type PendingAgreementAction = "none" | "google-signup" | "facebook-signup" | "manual-signup";

  const dispatch = useAppDispatch();
  const searchParams = useSearchParams();
  const isPhotographerSignup = searchParams.get("role") === "photographer";
  ...
  const startFacebookSignupFlow = () => {
    ...
    window.location.href = `${baseUrl}/auth/facebook?${query.toString()}`;
  };
  ...
  const performManualSignUp = async () => {
    ...
    try {
      const response = await signUp({
        ...
        fromDemoBonus,
        requestedRole: isPhotographerSignup ? "PHOTOGRAPHER" : "USER",
        photographerProfile: isPhotographerSignup
          ? {
      ...
      if (typeof err === "object" && err !== null && "response" in err) {
        ...
        if (response?.data?.details?.length > 0) {
          ...
          if (detail.path?.includes("password") && detail.code === "too_small") {
            setError("Password must be at least 8 characters.");
            return;
          }
        }
  ...
  const handleFacebookSignUp = () => {
    if (!hasAcceptedAllRequiredRegistrationAgreements()) {
      setRegistrationAgreementError("You must accept all required registration agreements to continue.");
      setError("You must accept all required registration agreements to continue.");
      setPendingAgreementAction("facebook-signup");
      setShowRegistrationModal(true);
      return;
    }
    setPendingAgreementAction("none");
    trackSignUp("facebook");
    startFacebookSignupFlow();
  };
      ...
      <div className="w-full max-w-md p-8 space-y-8 rounded-2xl bg-white shadow-lg border border-slate-200">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-slate-900">
            {isPhotographerSignup ? "Create your photographer account" : "Create your account"}
          </h1>
          <p className="text-slate-600 mt-2">
            {isPhotographerSignup
              ? "Join the marketplace and submit your photographer profile in one step"
              : "Join ElevateSpaces today"}
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div className="space-y-4 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4">
              <div>
                ...
                <p className="text-xs text-indigo-800 mt-1">
                  These details help the admin review your application faster.
                </p>
              </div>
              ...
              <textarea
                ...
                className="min-h-28 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600"
                placeholder="Short professional bio"
              />

              <textarea
                name="shortPitch"
                value={photographerForm.shortPitch}
                onChange={handlePhotographerChange}
                className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600"
                placeholder="Why should clients hire you?"
              />

              <textarea
                name="gearDescription"
                value={photographerForm.gearDescription}
                onChange={handlePhotographerChange}
                className="min-h-24 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm outline-none focus:border-indigo-600"
                placeholder="Camera / gear / equipment details"
              />
            </div>
          )}
          ...
          >
            {loading ? (
              ...
              isPhotographerSignup ? "Apply as Photographer" : "Sign up"
            )}
          </Button>
        ...
        <AgreementChecklistModal
          ...
          onConfirm={() => {
            ...
            if (actionToContinue === "facebook-signup") {
              startFacebookSignupFlow();
              return;
            }
