...
const OAUTH_ERROR_MAP: Record<string, string> = {
  oauth_account_not_found: "You must sign up first before signing in with Google.",
  oauth_agreements_required: "You must accept registration agreements before signing up with Google.",
  auth_failed: "Google authentication failed. Please try again.",
  server_error: "Google authentication failed due to a server error. Please try again.",
};

export default function SignInFormClient({ initialOauthError, initialOauthProvider }: SignInFormClientProps) {
  const dispatch = useAppDispatch();
  const router = useRouter();

  const initialErrorMessage = initialOauthError
    ? (OAUTH_ERROR_MAP[initialOauthError] || "Google authentication failed. Please try again.")
    : null;

  const showCreateAccountFirstText =
    initialOauthError === "oauth_account_not_found" && initialOauthProvider === "google";
  ...
  const handleGoogleLogin = () => {
    window.location.href = `${process.env.NEXT_PUBLIC_BACKEND_API}/auth/google?intent=signin`;
  };
    ...
    <div className="flex justify-center items-center min-h-screen bg-[#FAF7F2]">
      <div className="w-full max-w-md p-8 space-y-8 rounded-2xl bg-white shadow-lg border border-cream-200">
        <div className="text-center">
          ...
          <p className="text-cream-800/70 mt-2">Sign in to your Elevate Spaces AI account</p>
        </div>
        ...
        <div className="mt-6">
          ...
          <div className="mt-6 grid grid-cols-1 gap-3">
            ...
            {/* <Button variant="outline" className="h-11 border-cream-200 hover:bg-cream-50 text-cream-800/80 hover:border-cream-200" aria-label="Sign in with Apple">
            ...
            <Button variant="outline" className="h-11 border-cream-200 hover:bg-cream-50 text-cream-800/80 hover:border-slate.400" aria-label="Sign in with Facebook">
        ...
        <div className="text-center pt-4">
          <p className="text-cream-800/70 text-sm">
            Don't have an account? <Link href="/sign-up" className="font-semibold text-brand-500 hover:text-brand-600 hover:underline transition-colors">Sign up</Link>
          </p>
